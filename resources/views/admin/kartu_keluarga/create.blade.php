        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('admin.kartu_keluarga.store') }}" method="POST">
                        @csrf

                        <!-- Nomor KK -->
                        <div class="mb-4">
                            <label for="nomor_kk" class="block text-sm font-medium text-gray-700">Nomor KK</label>
                            <input type="text" id="nomor_kk" name="nomor_kk" value="{{ old('nomor_kk') }}" required
                            ...
                            <select id="kepala_keluarga" name="kepala_keluarga" required
                                ...
                                @foreach ($datawargas as $warga)
                                    <option value="{{ $warga->id }}"
                                        {{ old('kepala_keluarga') == $warga->id ? 'selected' : '' }}>
                                        {{ $warga->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <!-- Anggota Keluarga -->
                        <div class="mt-6">
                            <label for="anggota" class="block text-sm font-medium text-gray-700">Anggota
                                Keluarga</label>
                                ...
                                @foreach ($datawargas as $warga)
                                    <div class="flex items-center">
                                        <input type="checkbox" id="anggota-{{ $warga->id }}" name="anggota[]"
                                            value="{{ $warga->id }}"
                                            {{ in_array($warga->id, old('anggota', [])) ? 'checked' : '' }}
